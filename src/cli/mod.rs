...
use crate::types::{SearchOptions, SortSpec, SortField, SortOrder, ExportFormat, SizeFormat, DateFormat};
use std::sync::Arc;
    ...
    while i < args.len() {
        ...
        match arg.as_str() {
            ...
            "-sort" => {
                i += 1;
                if i < args.len() {
                    opts.sort = crate::engine::sorter::parse_sort_name(&args[i]);
                }
            }
            "-instance" => {
                i += 1;
...
pub fn run_cli(opts: &CliOptions, engine: &Arc<Engine>) -> Result<i32, String> {
    ...
    for entry in &results {
        if opts.highlight {
            let highlighted = crate::engine::searcher::SearchEngine::highlight_text(
...
pub fn print_help() {
    println!("Everything ES (文件检索助手 CLI)");
