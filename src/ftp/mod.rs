...
impl FtpServer {
    pub fn new() -> Self {
        Self { port: 21, username: "user".to_string(), password: "user".to_string(), root_dirs: Vec::new() }
    }
    pub fn with_port(mut self, port: u16) -> Self { self.port = port; self }
    pub fn with_auth(mut self, u: String, p: String) -> Self { self.username = u; self.password = p; self }
    pub fn with_root(mut self, dirs: Vec<PathBuf>) -> Self { self.root_dirs = dirs; self }
        ...
        match reader.read_line(&mut line) {
            ...
            Ok(_) => {
                ...
                let resp = match command.as_str() {
                    "USER" => {
                        if args == username { authenticated = true; "331 用户名正确，需要密码".to_string() }
                        else { "530 用户名错误".to_string() }
                    }
                    "PASS" => {
                        if authenticated && args == password { authenticated = true; "230 登录成功".to_string() }
                        else { authenticated = false; "530 密码错误".to_string() }
                    }
                    "SYST" => "215 UNIX Type: L8".to_string(),
                    "FEAT" => "211 支持: PASV LIST RETR SIZE".to_string(),
                    "TYPE" => "200 类型设置成功".to_string(),
                    "PWD" => format!("257 \"{}\" 是当前目录", current_dir.display()),
                    "CWD" => { current_dir = PathBuf::from(args); "250 目录改变成功".to_string() }
                    "QUIT" => { let _ = writeln!(stream, "221 再见"); break; }
