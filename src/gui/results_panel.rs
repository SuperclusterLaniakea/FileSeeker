...
use crate::engine::indexer::format_size;
use crate::types::{SortField, SortOrder};
use crate::gui::app::EverythingApp;
...
fn render_detail(ui: &mut egui::Ui, app: &EverythingApp) -> Option<ResultAction> {
    ...
    TableBuilder::new(ui)
        ...
        .header(22.0, |mut header| {
            let headers = [("名称",SortField::Name),("路径",SortField::Path),("大小",SortField::Size),("扩展名",SortField::Extension),("修改日期",SortField::DateModified)];
            for (name, field) in &headers {
                let arrow = if *field == app.sort_field {
                    match app.sort_order { SortOrder::Ascending => " ↑", SortOrder::Descending => " ↓" }
                } else { "" };
                let text = format!("{}{}", name, arrow);
                header.col(|ui| {
                    let resp = ui.button(RichText::new(text).strong().size(12.0));
                    if resp.clicked() { action = Some(ResultAction::Sort(*field)); }
                });
            }
        })
        .body(|body| {
            body.rows(22.0, total, |mut row| {
                ...
                row.col(|ui| {
                    ...
                    let resp = ui.add(label);
                    if resp.double_clicked() { action = Some(ResultAction::Open(row_idx)); }
                    if resp.secondary_clicked() {
                        let pos = ui.input(|i| i.pointer.interact_pos());
                        ui.ctx().memory_mut(|mem| {
                            mem.data.insert_temp(ctx_idx_id, row_idx);
                ...
                .show(ui.ctx(), |ui| {
                    ...
                    Frame::none()
                        ...
                        .show(ui, |ui| {
                            ...
                            ui.separator();
                            if ui.button("打开文件").clicked() { action = Some(ResultAction::Open(idx)); ui.ctx().memory_mut(|mem| mem.close_popup()); }
                            if ui.button("打开文件夹").clicked() { action = Some(ResultAction::OpenPath(idx)); ui.ctx().memory_mut(|mem| mem.close_popup()); }
                            if ui.button("重命名").clicked() {
                                if let Some(e) = app.results.get(idx) {
                                    #[cfg(windows)] { let _ = std::process::Command::new("explorer").args(&["/select,", &e.full_path.to_string_lossy().as_ref()]).spawn(); }
                                }
                                ui.ctx().memory_mut(|mem| mem.close_popup());
                            }
                            if ui.button("复制文件名").clicked() { action = Some(ResultAction::CopyName(idx)); ui.ctx().memory_mut(|mem| mem.close_popup()); }
                            if ui.button("复制完整路径").clicked() { action = Some(ResultAction::CopyPath(idx)); ui.ctx().memory_mut(|mem| mem.close_popup()); }
...
fn trunc_cjk(text: &str, max: usize) -> String {
    ...
    if cs.len() <= max { text.to_string() } else { format!("{}...", cs[..max.saturating_sub(3)].iter().collect::<String>()) }
}

fn get_path_display(entry: &crate::types::FileEntry) -> String {
    let p = entry.parent_path.to_string_lossy();
    if p.is_empty() || p == "\\" || p == "/" {
        if let Some(pp) = entry.full_path.parent() {
            let ps = pp.to_string_lossy();
            if ps.is_empty() || ps == "\\" || ps == "/" { entry.full_path.to_string_lossy().chars().take(3).collect() } else { ps.to_string() }
        } else { String::new() }
    } else { p.to_string() }
}
