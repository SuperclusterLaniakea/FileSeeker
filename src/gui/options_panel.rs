...
pub fn render_options(ui: &mut egui::Ui, app: &mut EverythingApp) {
    ...
    egui::SidePanel::left("opt_tabs").resizable(false).min_width(120.0).show_inside(ui, |ui| {
        ui.vertical(|ui| {
            let tabs = [
                ("[通用]", OptionsTab::General), ("[搜索]", OptionsTab::Search),
                ("[结果]", OptionsTab::Results), ("[共享]", OptionsTab::QuickShare),
                ("[高级]", OptionsTab::Advanced), ("[帮助]", OptionsTab::Help),
            ];
        ...
        if ui.button(if app.http_enabled { "⏹ 停止" } else { "▶ 启动" }).clicked() {
            ...
            if app.http_enabled {
                ...
                std::thread::spawn(move || {
                    let mut server = crate::http_server::HttpServer::new(engine).with_port(port);
                    let _ = server.start();
                });
            }
        }
    ...
    ui.horizontal(|ui| {
        ...
        if ui.button(if app.ftp_enabled { "⏹ 停止" } else { "▶ 启动" }).clicked() {
            ...
            if app.ftp_enabled {
                ...
                std::thread::spawn(move || {
                    let mut server = crate::ftp::FtpServer::new()
                        .with_port(port)
                        .with_auth(u, p)
                        .with_root(dirs);
                    let _ = server.start();
