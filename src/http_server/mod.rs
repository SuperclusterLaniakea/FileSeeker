...
use crate::types::{SearchOptions, SortSpec, SortField, SortOrder};
...
{}
    ...
    fn format_results_html(results: &[crate::types::FileEntry]) -> String {
        if results.is_empty() {
        ...
        for entry in results.iter().take(100) {
            ...
            let size_str = crate::engine::indexer::format_size(entry.size);
